...
async def run_inference(
    ...
    input_temp_files = []
    output_path = None # Hold the path to the output file
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as spk_file:
            ...
            input_temp_files.append(spk_audio_path)
        ...
        if emo_audio_prompt:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as emo_file:
                ...
                input_temp_files.append(emo_audio_path)
        ...
        # NOTE: We do NOT add the output_path to the input_temp_files list.
        # It will be handled by the BackgroundTask in the FileResponse.
        ...
        # The infer method returns a generator, we need to consume it.
        # The result (path or data) is yielded by the generator.
        for _ in tts_model.infer_generator(
            ...
            **generation_kwargs
            ...
            # The generator in infer_v2.py yields the path when done.
            # We just need to consume the generator to ensure it runs to completion.
            pass
        
        print(f"INFO:     Inference successful. Audio saved to temporary file: {output_path}")

        # --- Return Audio File ---
        # Use a BackgroundTask to delete the file *after* the response is sent.
        return FileResponse(
            path=output_path,
        ...
        # In case of any error, also clean up the output file if it was created
        ...
        # --- Cleanup ---
        # Clean up ONLY the INPUT temporary files.
        for path in input_temp_files:
...
async def run_inference(
    ...
    try:
        ...
        # --- Return Audio File ---
        # The FileResponse will stream the file and we can set it up to be deleted after sending.
        return FileResponse(
            path=output_path,
            media_type='audio/wav',
            filename='generated_speech.wav'
        ...
        # --- Cleanup ---
        # Clean up all temporary files created during the request

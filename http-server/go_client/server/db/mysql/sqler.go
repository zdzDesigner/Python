...
func (s *Sql) f_t_keys(keys ...string) []string {
	...
	for _, k := range keys {
		retkeys = append(retkeys, fmt.Sprintf("`%s`", k))
	}
	return retkeys
}
...
func TableColumn(tname string) ([]Column, error) {
	var list []Column
	SQL := fmt.Sprintf("SELECT `COLUMN_NAME`,`DATA_TYPE`,`COLUMN_COMMENT` FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='%s'", tname)
	err := Query(SQL, func(rows *sql.Rows) error {
		var col Column
		if err := rows.Scan(&col.Field, &col.Type, &col.Comment); err != nil {
			return err
		}
...
func TableStatistics(tname string, keys []string) ([]string, error) {
	...
	SQL := fmt.Sprintf("SELECT %s FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_NAME='%s'", keystr, tname)
	return quick(SQL)
}
...
func TableIndexs(tname string) ([]IndexField, error) {
	var list []IndexField
	SQL := fmt.Sprintf("SELECT DISTINCT `INDEX_NAME` FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_NAME='%s'", tname)
	err := Query(SQL, func(rows *sql.Rows) error {
		var col IndexField
...
// tables 获取所有表
func tables() ([]string, error) { return quick("SHOW TABLES") }
...
// Lastid 获取最后插入 ID
func Lastid() ([]string, error) { return quick("SELECT LAST_INSERT_ID()") }

...
const SectionList = () => {
  ...
  const saveNewSection = async (tempId, newName) => {
    ...
    try {
      ...
      if (response.code === 0) {
        ...
        setSections(prev => 
          prev.map(section => 
            section.id === tempId ? response.data : section
          )
        );
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 h-full flex flex-col" style={{ width: '200px' }}>
              ...
              >
                {formData.id === section.id ? (
                  <input
                    ...
                    className="text-sm border border-transparent rounded px-0 py-0 flex-1 outline-none ring-0 truncate bg-transparent h-5"
                    autoFocus
                  />
                ) : (
                  <span 
                    className="text-sm text-gray-700 truncate flex-1 cursor-pointer hover:text-blue-600 transition-colors leading-5"
